	...
	useEffect(() => {
		const userCookies =
			decodeURIComponent(window.document.cookie)
				?.split(';')
				?.find?.((item) => item.indexOf('FA:user') > -1)
				?.split('=')[1] ?? null;
				...
				<li className='mt-8 md:mt-0'>
					{User ? (
						<a
							...
							href={linkCTA}
							className='hover:bg-blue-500 transition-all duration-300 text-white hover:text-gray-400 text-lg px-6 py-3 font-medium ml-6 inline-flex items-center'>
